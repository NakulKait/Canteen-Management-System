...
//     if (data.length === 0)
//       return res.send(
//         result.createResult("No feedback found for this order", null)
//       );
...
//     if (data.length === 0)
//       return res.send(
//         result.createResult("No feedback found for this user", null)
//       );
...
// GET /feedback/order/:order_id - Feedback for an order
router.get("/order/:order_id", (req, res) => {
  const { order_id } = req.params;
  const sql = `
    SELECT feedback_id, user_id, order_id, rating, comment, created_at
...
// GET /feedback/user/:user_id - Feedback by a user
router.get("/user/:user_id", (req, res) => {
  const { user_id } = req.params;
  const sql = `
    SELECT feedback_id, user_id, order_id, rating, comment, created_at
